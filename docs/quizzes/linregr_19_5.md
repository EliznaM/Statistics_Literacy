...
QUIZ FOR CORRELATION AND LIN REGR, 19.5
...
for Covid-19 vs people not receiving treatment.  
b) The experimental groups in this analysis were people with severe 
Covid-19 vs people with milder disease.  
...
had an adjusted p-value (FDR) below 0.05.  
...
p-values smaller to filter out false positives.  
...
p-values on a scale that makes plotting easier.  
...
in the mild cases, and the difference was statistically significant.  
...
the blue dots, and the difference was statistically significant.   
j) In figure 2, the p-values represent the results of comparisons of every gene
with every other gene, within the severe disease group.  
